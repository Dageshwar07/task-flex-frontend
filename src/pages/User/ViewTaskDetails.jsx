...
/>
            ...
            <div className="mt-2">
              ...
              {task?.todoChecklist?.map((
                item, index) => (
                <TodoChecklist key={`todo_${index}`}
                  text={item.text} isChecked={item?.completed}
            ...
            {task?.attachments?.length > 0 && (
              <div className="mt-2 ">
                ...
                {task?.attachments?.map((link, index) => (
                  <Attachment key={`link_${index}`} link={link} index={index}
...
const Attachment = ({ link, index, onClick, key }) => {
  return <div key={key} className="flex justify-between bg-gray-50 border border-gray-100 px-3 py-2 rounded-md mb-3 mt-2 cursor-pointer" onClick={onClick}>
    ...
    <LuSquareArrowOutUpLeft className="text-gray-400" />
  </div>
}
